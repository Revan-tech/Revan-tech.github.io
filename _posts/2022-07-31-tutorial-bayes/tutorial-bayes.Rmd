...
Rozkład IQ jest, zgodnie z teorią, rozkładem normalnym. Dlatego $P(Dane|Hipoteza)$ możemy zamodelować jako prawdopodobieństwo otrzymania danych pod warunkiem określonych wartości średniej ($\mu$) i odchylenia standardowego ($\sigma$) rozkładu normalnego, które nazywamy parametrami. Gdy mamy dużo parametrów, zwykle dla wygody oznaczmy je literką $\theta$, która symbolizuje wektor parametrów. 
