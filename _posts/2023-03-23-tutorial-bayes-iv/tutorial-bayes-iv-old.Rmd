...
$$\bar{x_{i}} \sim N(\theta_j , \epsilon_j)$$
Przypomnijmy sobie, że estymatorem odchylenia standardowego średnich, czyli błędu standardowego, jest $\epsilon_j = \frac{\sigma_j}{\sqrt{n_j}}$, gdzie $n_j$ to liczebność danej grupy.
...
$$P(\theta_j|\bar{x_{j}}) \propto P(\bar{x_{j}}|\theta_j)P(\theta_j)$$
...
$$P(\theta_i|\bar{x_{j}}) \propto  \exp\left(-\frac{(\bar{x}_j - \theta_j)^2}{2\epsilon_i^2}\right)\exp\left(-\frac{(\theta_j-\mu)^2}{2\tau^2}\right)$$
...
$$\hat{\theta_j} = \frac{\tau^2 \bar{x}_j + \epsilon_j^2\mu}{\tau^2 + \epsilon_j^2} = \left(\frac{\tau^2}{\tau^2 + \epsilon_j^2}\right)\bar{x}_j + \left(\frac{\epsilon_j^2}{\tau^2 + \epsilon_j^2}\right)\mu$$
$$\frac{1}{S_j^2} = \frac{1}{\tau^2} + \frac{1}{\epsilon_j^2}$$
