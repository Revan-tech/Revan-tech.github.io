---
title: "Statystyka Zaawansowana w Środowisku R"
description: |
  Dlaczego do wielokrotnych pomiarów  nie możemy stosować standardowych metod statystycznych?
...
Gdy chcemy oszacować związek liniowy pomiedzy zmienną Y i X wykorzystujemy taką włłaśnie funckję liniową, szukając takich parametrów $a$ i $\beta$, które zminimalizują nam średni kwadrat błędu $E(\epsilon^2)$. 
...
plot2 <- ggplot(data_averaged, aes(x = mean_signal, y = mean_response)) +
  geom_point() + theme_bw() +xlab(" Srednia szybkość pisania") + ylab(" Średnia ilość błędów")
...
Nas jednak interesuje to, czy osoby z kiedy mają wyższą amplitudę sygnału dają też wyższą ocenę (efekt wewnątrzobiektowy). Możemy spróbować po prostu skorelowaćpojedyńcze pomiary pobudzenia i amplitudy ze sobą. Niestety ta metoda nie zadziałałaby. Dlaczego? Zasymulowałem takie dane:
...
```{r echo=FALSE, layout="l-body-outset", tidy=FALSE}
plot1 <- ggplot(data) + geom_point(aes(x = signal, y = response, colour =
  as.character(participant))) + theme_bw() + 
  theme(legend.position="none") + geom_smooth(mapping = aes(x = signal, y = response),method = "lm", se = F)
...
Jeśli spojrzymy na wykres po lewej, zobaczymy różnokolorowe zgrupowane punkty. Te kolory to poszczególni badani. Jeśli się im przyjrzymy zauważymy, że są lekko przechylone w prawo, co sugeruje związek liniowy. Jednak badani są rozrzuceni po całym wykresie (duża wariancja międzyobiektowa), ponieważ występuje duże zróżnicowanie pomiędzy ich bazowymi (średnimy) amplitudami i ocenami. Wobec tego linia regresji jest płaska.

Wykres ich średnich także pokazuje brak związku, ponieważ te bazowe wartości nie są ze sobą związane.

Jest to sytuacja, którą stosunkowo często obserwujemy. Mamy na przykład dwie osoby, osoba A jest bardziej reaktywna niż osoba B, co oznacza, że średnio będzie bardziej pobudzona i będzie miała większą reakcję skórno galwaniczną niż osoba B. Jednak u obu osób istnieje zależność taka, że **im większe pobudzenie tym większa reakcja skórno galwaniczna względem ich referencyjnego (bazowego, średniego) poziomu pobudzenia**. 
...
A gdybyśmy tak spróbowali (dosyć chałupniczo) od wartości naszych zmiennych odjąć średnią dla danego badanego?

```{r echo=FALSE, tidy=FALSE}
data = data %>% group_by(participant) %>% mutate(signal_demeaned = signal - mean(signal), 
response_demeaned = response - mean(response))

ggplot(data, aes(x = signal_demeaned, y = response_demeaned)) +
geom_point(aes(colour = as.character(participant))) + theme_bw() +  theme(legend.position="none") + geom_smooth(method = "lm", se = F)
```

Lepiej, co nie? Jak więc zbudować taki model liniowy, który usunie nam błąd wynikający z wariancji międzyobiektowej?

Nasz model regresji musimy zmodyfikować tak by brał pod uwagę średnie amplitudy badanych modelując je jako kolejne źródło błędu. 
